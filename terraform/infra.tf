...
resource "ovh_cloud_project_database" "db_eductive10" {
  engine       = "mysql"
  flavor       = "db1-4"
...
resource "ovh_cloud_project_database_user" "eductive10" {
  service_name = ovh_cloud_project_database.db_eductive10.service_name
  engine       = ovh_cloud_project_database.db_eductive10.engine
  name         = "eductive10"
  cluster_id   = ovh_cloud_project_database.db_eductive10.id
}

output "db_username" {
  value = ovh_cloud_project_database_user.eductive10.name
}

output "db_password" {
  value     = "password121996!"
  sensitive = true
}
  ...
  content  = templatefile("templates/inventory.tmpl",
    {
      ...
      db_hostname  = split("/",split("@",ovh_cloud_project_database.db_eductive10.endpoints[0].uri)[1])[0]
      db_username  = ovh_cloud_project_database_user.eductive10.name
      db_password  = ovh_cloud_project_database_user.eductive10.password
